...
pub const Nvenc = struct {
    ...
    pub fn init(cu: *const cuda.Cuda, fps: u32) !Nvenc {
        ...
        // Set all bitfield_flags explicitly (matching FFmpeg's nvenc_setup_av1_config):
        av1.bitfield_flags = (1 << 5) | (1 << 7); // repeatSeqHdr=1, chromaFormatIDC=1 (4:2:0)
        // Color metadata — BT.709 limited range, matching the explicit GPU shader
        // conversion in color_conversion.zig (same matrix as gpu-screen-recorder).
        av1.colorPrimaries = 1; // BT.709
        av1.transferCharacteristics = 1; // BT.709
        av1.matrixCoefficients = 1; // BT.709
        av1.colorRange = 0; // limited range (16-235)

        const buffer_format: u32 = NV_ENC_BUFFER_FORMAT_NV12;

        // Initialize encoder — NV12 input from explicit color conversion shader.
