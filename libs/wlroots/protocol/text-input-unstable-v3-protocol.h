...
struct zwp_text_input_v3_interface {
	 ...
	 * the enter event.
	 *
	 * This request must be issued every time the focused text input
	 ...
	 *
	 * Clients must not enable more than one text input on the single
	 * seat and should disable the current text input before enabling
	 * the new one. Requests to enable a text input when another text
	 * input is enabled on the same seat must be ignored by compositor.
	 *
	 ...
	 * applied on the next zwp_text_input_v3.commit request, and stay
	 ...
	 *
	 * Values set with this request are double-buffered. They will get
	 * applied on the next zwp_text_input_v3.commit request. Subsequent
	 ...
	 * from each zwp_text_input_v3 object and use the count as the
