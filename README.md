...
- **Zero unnecessary latency** — GPU-direct capture, hardware encode, P2P WebRTC transport. No compositor round-trip, no CPU encode, no relay server (in the common case). Not actually zero — there's still capture interval, encode time, and network transit.
...
- **Zero servers to manage** — Signaling runs on Cloudflare Workers (serverless). Media flows P2P via STUN. No VMs, no containers, no infrastructure to maintain.
